...
export default function() {
    ...
    //if not a master or an instance
		sketch.UI.alert(pluginName,'Select one symbol master or instance.');
		return false;
	}

  //get the master (whether it is current selection or not)
  ...
  var symbolInstances = getSymbolMasters(document,symbolMaster);
...
function displayInstances(parent,instances,type) {
  ...
  console.log(instances)
    ...
    //instance = instance.sketchObject;
 ...
 function getSymbolMasters(source, symbolMaster) {
	var symbolMasters = NSMutableArray.array();
  var symbolMasterLibrary = symbolMaster.getLibrary();

  console.log('children')
  console.log(source.sketchObject.pages()[0].children()[0])


  var symbolReferences = symbolMasterLibrary.getImportableSymbolReferencesForDocument(document);

  symbolReferences.forEach(function(symbol) {
    if(symbol.name == symbolMaster.name) {
      symbolMasters.addObject(symbol.sketchObject)
    }
  });
...
function getSymbolInstances(source,symbolMaster2) {
	var symbolInstances = NSMutableArray.array();

	source.sketchObject.pages().forEach(function(page){
    //filter for class = symbol and symbol id matching symbolmaster
    //symbolMaster.objectID refers to the child's symbolsMaster
		var predicate = NSPredicate.predicateWithFormat('className == %@ && symbolMaster.objectID == %@','MSSymbolInstance',symbolMaster2.sketchObject.objectID());
